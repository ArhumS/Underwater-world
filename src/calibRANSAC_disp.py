...
# in a JSON file. This will run for a long time, potentially. You can configure
...
def doDisplay(args, json, pts) :
  ...
  for z in pts :
    ...
    for p in ptsl :
      cv2.circle(imgl, (p[0], p[1]), 5, (255, 0, 0)) 
    for p in ptsr :
      cv2.circle(imgr, (p[0], p[1]), 5, (255, 0, 0)) 
    ...
    for p in lp :
      cv2.circle(imgl, (p[0], p[1]), 5, (0, 255, 0)) 
    for p in rp :
      cv2.circle(imgr, (p[0], p[1]), 5, (0, 255, 0)) 
